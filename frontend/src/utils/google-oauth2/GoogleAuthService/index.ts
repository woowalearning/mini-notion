...
export default class GoogleAuthService {
  clientProvider: GapiClientProvider

  sessionStorage: SessionStorage

  constructor(clientProvider: GapiClientProvider, sessionStorage: SessionStorage) {
   ...
   */
  getAuthInstance(): Promise<Omit<gapi.auth2.GoogleAuth, 'then'>> {
    return this.clientProvider
      .getClient()
      .then(({ authInstance, ...props }) => {
        console.log(props);
        return authInstance;
      });
  }
   ...
   */
  getCurrentUser() {
    return this.getAuthInstance().then((authInstance) => authInstance.currentUser.get());
  }
   ...
   * @return {boolean}
   */
  hasGrantedRequestedScopes(currentUser: gapi.auth2.GoogleUser) {
   ...
   * @property {boolean} [grantOfflineAccess=false]
   * Additionally gets permission from the user
   * to access the specified scopes offline via {@link GoogleAuthService#getOfflineAccessCode}
   */
  ...
  login({ grantOfflineAccess = false } = {}) {
    return this.getAuthInstance()
      .then((authInstance) => authInstance.signIn().then((currentUser) => {
   ...
   * @return {Promise<void>}
   */
  listenUserSignIn(callback: (signedIn: boolean) => any) {
